...
def getinitialPopulation(length, length0, length1, length2, length3, populationSize):
    ...
    chromsomes0 = np.zeros((populationSize, length0), dtype=np.int_)
    chromsomes1 = np.zeros((populationSize, length1), dtype=np.int_)
    chromsomes2 = np.zeros((populationSize, length2), dtype=np.int_)
    chromsomes3 = np.zeros((populationSize, length3), dtype=np.int_)
...
def getPopulation(population, populationSize):
    population0 = np.zeros((populationSize, 6), dtype=np.int_)
    population1 = np.zeros((populationSize, 2), dtype=np.int_)
    population2 = np.zeros((populationSize, 2), dtype=np.int_)
    population3 = np.zeros((populationSize, 1), dtype=np.int_)
    for popusize in range(populationSize):
        # np.random.randit()产生[0,2)之间的随机整数，第三个参数表示随机数的数量
...
# 得到每个个体的适应度值及累计概率
def getFitnessValue(func, decode, w1, v1, w2, v2,x_train, y_train):
    # 得到种群的规模和决策变量的个数
    popusize = decode.shape[0]  # 100
    # 初始化适应度值空间
    fitnessValue = np.zeros((popusize, 1))     # (100,1)
    for popunum in range(popusize):
        fitnessValue[popunum] = func(x_train, y_train, w1, v1, w2, v2, 3, 2, 1, popusize, y_train)[-1]
    # 得到每个个体被选择的概率
...
# 适应度函数，神经网络训练误差最小为
def fitnessFunction(dataset, labelset, temp1, temp2, temp3, temp4, inputnum , hiddennum, outputnum, num,y_train):
    # x为步长
    x = 0.05
...
def test_process(dataset, labelset, weight1, weight2, value1, value2):
    ...
    print(dataset.shape, labelset.shape, weight1.shape, weight2.shape, value1.shape, value2.shape)
    pre_data = []
    for i in range(len(dataset)):
        # 计算每一个样例通过该神经网路后的预测值
        inputset = np.mat(dataset[i]).astype(np.float64)
        ...
        # print('inputset, weight1, value1', inputset, weight1, value1)
        # print(inputset=[3, 1], weight1=[3,2], value1=[2,1])
        output3 = np.dot(output2, weight2) - value2
        # print('output2, weight2, value2, output3', output2, weight2, value2, output3)
        # print(output2=[2, 2], weight2=[2, 1], value2=[1, 1], output3=[2, 1])
...
def gabpnet(path, length):
#if __name__ == '__main__':
    # 要打开的文件名
    #path = './history_50_220107_231110.xlsx'
    ...
    # x, y = splitData(df)
    # 数据归一化
    x = df[['load', 'temperature', 'wind_speed']].values     # x = [2881, 3]
    x = np.reshape(x, (len(x)*3, 1))     # x = [8643, 1]
    ...
    # 分训练和测试集
    x_train, x_test, y_train, y_test = train_test_split(x_norm, y_norm, test_size=length * 2, shuffle=False)
    ...
    for generation in range(maxgeneration):
        ...
        # 得到适应度值和累计概率值
        evaluation, cum_proba = getFitnessValue(fitnessFunction, decode, W1, V1, W2, V2,x_train, y_train)
        # 选择新的种群
        ...
        # 适应度评估
        final_evaluation, final_cumprob = getFitnessValue(fitnessFunction, final_decode, W11, V11, W22, V22,x_train, y_train)
    ...
    # 训练
    for i in range(2):
        Weight1, Weight2, Value1, Value2 = fitnessFunction(x_train, y_train, weight1, weight2, value1, value2, 3, 2, 1, 0,y_train)
    # 预测
    pre = test_process(x_test, y_test, Weight1, Weight2, Value1, Value2)
    # print(x_test, y_test, Weight1, Weight2, Value1, Value2)
    #print(pre)
    ...
    a = list(pre_org.reshape(length * 2))
    b = a[:length]
    c = a[length:length * 2]
    pd.DataFrame(b).to_excel(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'result/5_GABPNet_test.xlsx'),
                         index=False, header=False)
    pd.DataFrame(c).to_excel(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'result/5_GABPNet_pred.xlsx'),
                         index=False, header=False)
#    # 显示测试图像
...
if __name__=='__main__':
    gabpnet('./history_50_220107_231110.xlsx')