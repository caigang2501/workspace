...
def parse_args():
    ...
    parser.add_argument('--train_labeled_file', type=str,
                        default='data_cartoon/tr_label.csv')
    parser.add_argument('--val_labeled_file', type=str,
                        default='data_cartoon/val_label.csv')
    parser.add_argument('--img_dir', type=str, default='data_cartoon/imgs_cartoon/')
...
if __name__ == '__main__':
    ...
    if args.resume:
        checkpoints = torch.load('data_cartoon/checkpoints/last_checkpoint.pt')
    ...
    for e in range(start_epoch+1, args.epochs+1):
        ...
        torch.save(states,
                   'data_cartoon/checkpoints/last_checkpoint.pt')
        if val_f1 > best_f1:
            best_f1 = val_f1
            torch.save(model.state_dict(),
                       'data_cartoon/checkpoints/weights/best_model.pt')
        if e % args.save_freq == 0:
            torch.save(model.state_dict(),
                       'data_cartoon/checkpoints/weights/epoch_{}_f1_{:.5f}.pt'.format(e, val_f1))
    log_file.close()
